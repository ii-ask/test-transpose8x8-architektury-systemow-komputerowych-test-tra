#include <errno.h>
#include <inttypes.h>
#include <stdint.h>
#include <stdio.h>
#include <stdlib.h>
#include <stdnoreturn.h>
#include <string.h>
#include <sys/time.h>

#define __noinline __attribute__((noinline))

extern uint64_t transpose8x8(uint64_t);

/* https://en.wikipedia.org/wiki/Xorshift#xorshift* */
static uint64_t random_u64(uint64_t *seed) {
  uint64_t x = *seed;
  x ^= x >> 12;
  x ^= x << 25;
  x ^= x >> 27;
  *seed = x;
  return x * 0x2545F4914F6CDD1DUL;
}

/* Naiwna implementacja w C - służy tylko do weryfikacji poprawności. */
static __noinline uint64_t transpose_naive(uint64_t x) {
  uint64_t res = 0;
  for (int row = 0; row < 8; row++) {
    for (int col = 0; col < 8; col++) {
      /* Jeśli bit na pozycji (row, col) jest zapalony */
      if (x & (1ULL << (row * 8 + col))) {
        /* Zapal bit na pozycji (col, row) w wyniku */
        res |= (1ULL << (col * 8 + row));
      }
    }
  }
  return res;
}

typedef union caller_regs {
  struct {
    uint64_t rbx;
    uint64_t r12;
    uint64_t r13;
    uint64_t r14;
    uint64_t rbp;
  };
  uint64_t reg[5];
} caller_regs_t;

#define save_caller_regs(regs)                                                 \
  asm volatile("mov %%rbx,  0(%0);"                                            \
               "mov %%r12,  8(%0);"                                            \
               "mov %%r13, 16(%0);"                                            \
               "mov %%r14, 24(%0);"                                            \
               "mov %%rbp, 32(%0);"                                            \
               : /* no outputs */                                              \
               : "r" (regs)                                                    \
               : "memory", "rbx", "r12", "r13", "r14", "rbp")

static void run(uint64_t arg) {
  uint64_t fast, slow;

  caller_regs_t before, after;
  save_caller_regs(&before);
  fast = transpose8x8(arg);
  save_caller_regs(&after);

  for (int i = 0; i < sizeof(caller_regs_t) / sizeof(uint64_t); i++) {
    if (before.reg[i] != after.reg[i]) {
      printf("transpose8x8(...) does not adhere to ABI calling convention!\n");
      exit(EXIT_FAILURE);
    }
  }

  slow = transpose_naive(arg);
  if (fast != slow) {
    printf("transpose8x8(0x%016lx) = 0x%016lx (your answer: 0x%016lx)\n", arg, slow, fast);
    exit(EXIT_FAILURE);
  }
}

int main(int argc, char *argv[]) {
  if (argc == 2) {
    uint64_t arg = strtoul(argv[1], NULL, 16);
    if (errno)
      goto fail;
    run(arg);
    return EXIT_SUCCESS;
  }

  if (argc == 3) {
    if (strcmp("-r", argv[1]))
      goto fail;

    int times = strtol(argv[2], NULL, 10);
    if (times < 0)
      goto fail;

    struct timeval tv;
    gettimeofday(&tv, NULL);

    uint64_t seed = tv.tv_sec + tv.tv_usec * 1e6;

    for (int i = 0; i < times; i++) {
      /* Dla transpozycji pełne losowe 64-bity są idealnym przypadkiem testowym */
      uint64_t val = random_u64(&seed);
      run(val);
    }

    return EXIT_SUCCESS;
  }

fail:
  fprintf(stderr, "Usage: %s [-r TIMES] [NUMBER]\n", argv[0]);
  return EXIT_FAILURE;
}


